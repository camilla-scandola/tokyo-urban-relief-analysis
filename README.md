...
A **linear regression with ward fixed effects** was applied to project green space trajectories through 2026

- **Target variable**: `park_percent_of_ward`  
- **Dataset**: 23 wards × 8 years (2016–2023) = 184 observations  
- **R² = 0.995 | MAE = 0.112**  
- Year coefficient: **+0.044** (average % green acquired per ward per year)  

Forecasts for 2024–2026 were generated by holding ward fixed effects constant and extrapolating the year forward

---

## REST API

A lightweight **Flask REST API** serves ward and building data from the MySQL database via SQLAlchemy

**Endpoints:**
- `GET /api/wards` —> all 23 wards with green space %, density, and area
- `GET /api/wards/<ward_name>` —>  full ward profile including flood risk and CASBEE data
- `GET /api/buildings` —> CASBEE buildings, filterable by ward, rank, and year
